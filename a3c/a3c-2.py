...
class Actor:
    def __init__(self, state_dim, action_dim, action_bound, std_bound):
        ...
        print('STD BOUND {}'.format(std_bound))
        ...
        while self.max_episodes >= GLOBAL_EPISODE_NUM:
            ...
            while not done:
                ...
                if len(state_batch) >= args.update_interval or done:
                    ...
                    discounted_rewards = self.n_step_td_target(
                        rewards, next_v_value, done)

                    print(len(state_batch))
                    print('REWARDS {}'.format(discounted_rewards))
                    print('STATES {} SHAPE {}'.format(state, states.shape))
                    print(states.shape)

                    advantages = discounted_rewards - \
                        self.critic.model.predict(states)

                    print('ADVANTAGES {}'.format(discounted_rewards))
